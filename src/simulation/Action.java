...
//    }
    ...
    private void attackIfPossible(ArrayList<GameObject> warriors, int turn, int a, int targetX, int targetY) {
        for (int i = 0; i < warriors.size(); i++) {
    ...
    public void scanForEnemy(ArrayList<GameObject> warriors, int turn) {
        ...
        for (int t = 0; t < board.size(); t++) {
            for (int a = 0; a < warriors.size(); a++) {
                if (t == 0 || (warriors.get(a).getStatus() == Status.NotMoved)) {
                    ...
                    // Close range attack
                    if (minDistance == 1) {
                        attackIfPossible(warriors, turn, a, targetX, targetY);
                    }

                    // Moving towards closest an enemy and charge attack
                    if (minDistance > 1 && minDistance <= 2 && warriors.get(a).getStatus() != Status.Attacked) {

//                        System.out.println("Before moving: " + warriors.get(a).getId() + " " + warriors.get(a).getStatus() + " x: " + warriors.get(a).getX() + " y: " + warriors.get(a).getY() + " targetX: " + targetX + " targetY: " + targetY);

                        moveIfPossible(warriors, a, allyToMove, x, y, targetX, targetY);

//                        System.out.println("After moving: " + warriors.get(a).getId() + " " + warriors.get(a).getStatus() + " x: " + warriors.get(a).getX() + " y: " + warriors.get(a).getY() + " targetX: " + targetX + " targetY: " + targetY);

                        Status status = warriors.get(a).getStatus();


                        if ((status == Status.MovedUp && targetX - warriors.get(a).getX() == -1) || (status == Status.MovedDown && targetX - warriors.get(a).getX() == 1)
                                || (status == Status.MovedLeft && targetY - warriors.get(a).getY() == -1) || (status == Status.MovedRight && targetY - warriors.get(a).getY() == 1))
                            attackIfPossible(warriors, turn, a, targetX, targetY);

                    }

                    // Moving towards closest an enemy
                    if (warriors.get(a).getStatus() == Status.NotMoved) {
                        moveIfPossible(warriors, a, allyToMove, x, y, targetX, targetY);
                    }
